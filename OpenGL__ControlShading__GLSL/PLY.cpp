...
static float diffuse[4] = {0.7, 0.7, 1.0, 1.0};
static float specular[4] = {0.5, 0.5, 0.5, 1.0};
static float shininess[1] = {100.0};
static float white[4] = {0.8, 0.8, 0.8, 1.0};
...
void PLYObject::draw()
{
  ...
  for (i = 0; i < nf; i++) {
    ...
    for (j = 0; j < 3; j++) {
      if (texture && hastexture)
        glTexCoord2fv(texcoords[faces[i][j]]);	// set texture coordinates
#ifdef INTERLEAVED
      if (!texture && hascolor)
